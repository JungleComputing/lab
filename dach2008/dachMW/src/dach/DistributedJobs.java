...
public class DistributedJobs implements JobProducer {
	...
	public DistributedJobs(List<Cluster> clusters, Collection<Problem> problems,
	...
	private List<FileInfo> produceJobs(Cluster c, Problem problem) { 
		...
		while (result == null) { 
			...
			try {
				result = client.list(c.master, dir, true);
				logger.info(c.master + " returned listing of " + result.size() + " files");
				break;
			} catch (Exception e) {
				logger.warn("Failed to list files on " + c.master, e);
			}
	...
	private void produceJobs(Problem problem) { 
		...
		for (Cluster c : clusters) {
			List<FileInfo> tmp = produceJobs(c, problem);			
			merge(files, tmp);			
		}		
