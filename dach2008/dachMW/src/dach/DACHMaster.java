...
public class DACHMaster implements ResultProcessor {
	...
	public DACHMaster(LinkedList<Cluster> workerClusters, LinkedList<Cluster> dataClusters) throws Exception { 
		...
		if (concurrent) { 
			...
			if (workerClusters.size() == 1) { 
				workerOptions.add(Integer.toString(workerClusters.getFirst().cores)); 
			}
		...
		master = new Master(this, workerClusters, workerOptions, homeDir, 
				dataClusters, dataDir, allowRemote, advancedQueue);
	}
	...
	private List<DACHJob> getJobs() { 
		...
		if (dfs) {
			producer = new DFSJobs(new File(dataDir), problems, locationExec, true);
		} else { 
			...
			try {
				producer = new DistributedJobs(dataClusters, problems, true);
			} catch (Exception e) {
	...
	public static void main(String [] args) { 
		...
		problems = getProblems(problemNames);
