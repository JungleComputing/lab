...
public class DACHWorker implements JobProcessor {
	...
	public DACHWorker() { 
		...
		try {
			worker = new Worker(this, serverAddress, hubAddresses, pool,
					node, location, masterID, preferSmall, timeout, concurrentWorkers);
		} catch (Exception e) {
	...
	*/
	...
	private void compare(DACHJob pair, DACHResult result) {
		...
		info(result, "Comparing '" + pair.beforeInfo.name + "' and '" + pair.afterInfo.name + "'\n");
    	...
    	if (dryRun) { 
			...
			String message = "Completed '" + before + "' and '" + after + "' in " + (end-start) 
				+ "ms. Total time " + result.getTime() + " ms.";
