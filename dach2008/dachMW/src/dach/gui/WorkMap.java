...
public class WorkMap extends JComponent 
    ...
    public WorkMap(LinkedList<ClusterStatistics> stats) {
    
    	int totalLines = stats.size() * 4 + 4;
    	long endTime = 0;
		long jobEndTime = 0;
...
//        	big.drawImage(map, 0, 0, null);
    ...
    private int drawCluster(Graphics big, ClusterStatistics c, int line) { 
    	...
    	for (NodeStatistics n : c.nodes) { 
    		...
    		for (NodeStatistics.Event e : n.events) { 
    			...
    			case STEAL_REPLY_EMPTY:			
    				...
    				big.fillRect((int) e.time+scaleW, myLine, scaleW, 1);
    				break;
    			case START_COMPUTE:
    				if (lastStart != -1) { 
